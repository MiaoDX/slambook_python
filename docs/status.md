...
| ch7 | Feature VO | Baseline complete | OpenCV | LightGlue/LoFTR/PyCOLMAP | ORB/SIFT interfaces, LightGlue/LoFTR adapters, PyCOLMAP reference reconstruction CLI, optional Rerun match/track logging, and legacy script status documented |
...
| ch9 | VO project | Baseline complete | OpenCV/SciPy | GTSAM/PyCOLMAP | Data model, frame feature extraction, VO coordinator, local-map tracking with motion-only BA, optional PyCOLMAP absolute-pose reference, trajectory/viz helpers, documented image layout, and smoke-tested monocular runner added |
...
| ch12 | Loop closure | Baseline complete | OpenCV descriptors | FAISS/hloc-style comparison | OpenCV descriptor builder, NumPy/FAISS retrieval, temporal filtering, and CLIs added |
