...
```bash
uv run python examples/ch12_loop_closure/build_descriptors.py \
...
```bash
uv run python examples/ch12_loop_closure/train_vocabulary.py \
  --images data/slambook/ch12 \
  --words 64 \
...
```bash
uv run python examples/ch12_loop_closure/retrieve_candidates.py \
